from custom_config import cfg, set_random_seed, QUESTION2LEVEL, LEVEL2QUESTION
...
def fold_assign(df_label, df):
    ...
    if cfg.use_polar:
        df = df.with_column(pl.col('session_id').map_dict(session2fold).alias('fold'))
    else:
...
def split_data(df, df_labels, fold = 0, level = '0-4'):
    ...
    if level == '0-4':
        ...
        past_level = ['0-4', '5-12', '13-22']
        
    if cfg.use_polar:
        trn = df.filter(pl.col('fold') != fold).filter(pl.col('level_group').is_in(past_level))
        val = df.filter(pl.col('fold') == fold).filter(pl.col('level_group').is_in(past_level))
    else:
        trn = df[(df['fold'] != fold) & (df['level_group'].isin(past_level))]
        val = df[(df['fold'] == fold) & (df['level_group'].isin(past_level))]
    ...
    trn_labels = df_labels[df_labels['fold'] != fold]
    val_labels = df_labels[df_labels['fold'] == fold]
