    ...
    for fold in cfg.training_folds:
        ...
        for i, level in enumerate(['0-4', '5-12', '13-22']):
            ...
            if cfg.mode == 'train':
                ...
                trn_embeddings_df.to_csv(os.path.join(cfg.model_dir, cfg.ver[:-1], cfg.ver[-1], f"{cfg.ver}_trn_embeddings_level_{level.replace('-', '_')}_fold_{fold}_w_pred.csv"))
                val_embeddings_df.to_csv(os.path.join(cfg.model_dir, cfg.ver[:-1], cfg.ver[-1], f"{cfg.ver}_val_embeddings_level_{level.replace('-', '_')}_fold_{fold}_w_pred.csv"))
