import os
from custom_config import cfg, set_random_seed
import pandas as pd
import polars as pl
...
def process_data(df, only_drop = False):
    ...
    Steps:
      ...
      2. Fill NaN values in float columns by -999.00
    '''
        ...
        if not only_drop:
            ...
            for c in df.columns:
                if df[c].dtype == pl.datatypes.Float64:
                    fill_null.append(pl.col(c).fill_null(-1.))
...
def feature_engineering_pl(df, group = '(0-4)', use_extra_features = True, feature_suffix = '', remaining_features = None):
    ...
    if remaining_features is None:
        ...
        drop_cols = list(null_features[null_features > 0.7].index) + ['level_group']
